...
{
    public static class RowValueCController
    {
        public static List<RowValue> GetRowValueByRowID(int id)
        {
            ...
            string query = "";
            query = "SELECT * FROM RowValuec WHERE RowID = " + id + " Order By TimeStamp";
        ...
        public static RowValue GetRowValueByRowIDAndTimeStamp(double rowID, double timeStamp)
        {
            ...
            MySqlConnection conn = connect.ConnectMySQL();
            string query = "";
            query = "SELECT * FROM RowValuec WHERE RowID = " + rowID + " AND TimeStamp=" + timeStamp;
        ...
        public static string Insert(RowValue rowValue)
        {
            ...
            string query = "insert into rowvaluec(RowID,Value,TimeStamp,Quater) values('"
                + rowValue.RowID + "','"
                + rowValue.Value + "','"
                + rowValue.TimeStamp + "','"
                + rowValue.Quater + "');";
            ...
            {
                query = "insert into rowvaluec(RowID,TimeStamp,Quater) values('"
                    + rowValue.RowID + "','"
                    + rowValue.TimeStamp + "','"
                    + rowValue.Quater + "');";
            }
        ...
        public static string Update(RowValue rowvalue)
        {
            DBConnect dBConnect = new DBConnect();
            MySqlConnection conn = dBConnect.ConnectMySQL();
            string query = "UPDATE Rowvaluec "
                + "SET value = '" + rowvalue.Value + "' "
                + "WHERE id = '" + rowvalue.ID + "'";
            if (rowvalue.Value == null)
            {
                query = "UPDATE Rowvaluec "
                + "SET value = NULL "
                + "WHERE id = '" + rowvalue.ID + "'";
            }
            MySqlCommand cmd = new MySqlCommand(query, conn);
